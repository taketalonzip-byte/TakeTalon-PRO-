    ...
    <div className="lg:pl-64">
      ...
      <main className="mx-auto max-w-[1600px] p-3 sm:p-5 space-y-4">
        ...
        {section === "overview" && <>
          ...
          ].map(([label, value, Icon, color]) => <div key={String(label)} className={`rounded-2xl border ${panel} p-3`}><div className="flex items-center justify-between"><p className={`text-[11px] font-bold ${muted}`}>{label}</p><Icon className={`h-3.5 w-3.5 ${color}`} /></div><p className="mt-1.5 text-lg font-black">{loading ? "—" : value}</p></div>)}</section>
