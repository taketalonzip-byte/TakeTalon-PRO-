...
type Wallet = {
  id: string; profile_id?: string; username?: string; email?: string; role?: string;
...
export default function FinanceAdminPortal({ currentUser, theme, lang }: FinanceAdminPortalProps) {
  ...
  const filteredWallets = useMemo(() => {
    const q = query.toLowerCase().trim();
    return (dashboard?.wallets || []).filter((row) => !q || [row.username, row.email, row.profile_id, row.role].some((v) => String(v || "").toLowerCase().includes(q)));
  }, [dashboard, query]);
...
function WalletTable({ rows, light, muted }: { rows: Wallet[]; light: boolean; muted: string }) { return <div className="max-h-[340px] overflow-auto"><table className="w-full min-w-[940px] text-left"><thead className={`sticky top-0 z-[1] ${light ? "bg-slate-50" : "bg-[#1f3d5c]"}`}><tr className={`text-[10px] uppercase tracking-wide ${muted}`}><th className="px-4 py-2.5">Profile</th><th className="px-4 py-2.5">Role</th><th className="px-4 py-2.5 text-right">Balance</th><th className="px-4 py-2.5 text-right">Available</th><th className="px-4 py-2.5 text-right">Reserved</th><th className="px-4 py-2.5">Updated</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t border-inherit text-[12px]"><td className="px-4 py-2.5"><p className="font-bold">{row.username ? `@${row.username}` : "Unknown profile"}</p><p className={`font-mono text-[10px] ${muted}`}>{row.email || row.profile_id || "—"}</p></td><td className="px-4 py-2.5"><span className={`rounded-md px-2 py-1 text-[10px] font-black uppercase ${row.role === "ADMIN" || row.role === "SUPER_ADMIN" || row.role === "OWNER" ? "bg-orange-100 text-orange-700" : "bg-slate-200 text-slate-600"}`}>{row.role || "USER"}</span></td><td className="px-4 py-2.5 text-right font-mono font-bold">{money(row.balance)}</td><td className="px-4 py-2.5 text-right font-mono text-[#3B6D99]">{money(row.available_balance)}</td><td className="px-4 py-2.5 text-right font-mono text-amber-500">{money(row.reserved_balance)}</td><td className={`whitespace-nowrap px-4 py-2.5 ${muted}`}>{date(row.updated_at)}</td></tr>)}{!rows.length && <TableEmpty cols={6} muted={muted} />}</tbody></table></div>; }
function UnmatchedTable({ rows, light, muted }: { rows: Unmatched[]; light: boolean; muted: string }) { return <div className="max-h-[340px] overflow-auto"><table className="w-full min-w-[700px] text-left"><thead className={`sticky top-0 z-[1] ${light ? "bg-red-50" : "bg-[#4a2930]"}`}><tr className={`text-[10px] uppercase tracking-wide ${muted}`}><th className="px-4 py-2.5">Phone normalized</th><th className="px-4 py-2.5 text-right">Total unmatched</th><th className="px-4 py-2.5 text-right">Deposits</th><th className="px-4 py-2.5">Last seen</th><th className="px-4 py-2.5">Registry status</th></tr></thead><tbody>{rows.map((row) => <tr key={row.phone_normalized} className="border-t border-inherit text-[12px]"><td className="px-4 py-2.5 font-mono font-bold">{row.phone_normalized}</td><td className="px-4 py-2.5 text-right font-mono font-bold text-amber-500">{money(row.total_unmatched_amount ?? row.parsed_amount)}</td><td className="px-4 py-2.5 text-right font-mono">{row.deposit_count || 1}</td><td className={`px-4 py-2.5 whitespace-nowrap ${muted}`}>{date(row.last_seen_at)}</td><td className="px-4 py-2.5"><span className="rounded-md bg-red-50 px-2 py-1 text-[10px] font-black text-red-600">UNMATCHED</span></td></tr>)}{!rows.length && <TableEmpty cols={5} muted={muted} />}</tbody></table></div>; }
