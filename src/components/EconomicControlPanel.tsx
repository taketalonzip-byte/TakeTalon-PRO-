...
type Category = "ACCESS" | "SUBSCRIPTION" | "DEPOSIT" | "WITHDRAWAL" | "FEES" | "EARNINGS" | "GAME" | "TREASURY" | "SYSTEM" | "AGENT_SERVICES";
...
function labelFor(rule: RuleRow, lang: string) {
  const labels: Record<string, string> = {
    ...
    agent_join_fee_scale_factor: lang === "sw" ? "Asilimia ya ada ya Become Agent" : "Become Agent fee percentage",
    agent_join_fee_fbu: lang === "sw" ? "Ada ya msingi ya Become Agent" : "Become Agent base fee",
...
export default function EconomicControlPanel({ currentUser, theme, lang, onBack, onAddNotification }: EconomicControlPanelProps) {
  ...
  const categoryRules = useMemo(() => rules.filter((rule) => rule.category === selectedCategory && (selectedCategory !== "GAME" && selectedCategory !== "AGENT_SERVICES" || (rule.control_mode === "SCALE" && rule.key !== "games_betting_minimum_scale_factor"))), [rules, selectedCategory]);
  ...
  const selectedRule = selectedCategory === "ACCESS"
    ...
    : selectedCategory === "AGENT_SERVICES"
      ? rules.find((rule) => rule.key === "agent_join_fee_scale_factor")
  ...
  const pendingAgentScale = pendingRequests.find((request) => request.rule_key === "agent_join_fee_scale_factor") ? Number(pendingRequests.find((request) => request.rule_key === "agent_join_fee_scale_factor")?.requested_value) : null;
  ...
  const approveRequest = async (requestId: string) => {
    ...
    if (error || (data as any)?.ok === false) {
      ...
      window.dispatchEvent(new Event("taketalon:business-rules-updated"));
      await loadRules();
    }
    setSaving(false);
  };
