-- TakeTalon PRO — expose wallet profile role to Finance Admin read model
-- Read-only addition: no wallet balances or permissions are changed.
...
BEGIN
  ...
  FROM (
    SELECT w.id,
           ...
           p.username,
           p.email,
           upper(coalesce(p.role, 'USER')) AS role,
           w.balance,
           w.available_balance,
           w.reserved_balance,
           w.updated_at
...
COMMENT ON FUNCTION public.finance_admin_dashboard(integer) IS
  'Finance Admin read model including wallet profile username, email, and role.';
